...
BLINK_ALIASES = ["ojoscerrados", "ojos cerrados"]
...
app.title("Avatar Controller + AI + STT + TTS + Blender HTTP")
app.geometry("1020x1180")

title = tb.Label(
    app,
    text="Avatar Assistant Controller",
...
rotation_slider_vars = {}
rotation_value_labels = {}
rotation_meta = {}
...
# ============================================================
# BLENDER HTTP COMMUNICATION
# ============================================================
def get_current_payload():
    params = {}

    for name, var in shape_slider_vars.items():
        params[name] = float(var.get())

    for name, var in rotation_slider_vars.items():
        params[name] = float(var.get())
...
def send_payload_to_blender(payload):
    data = requests.post(
        BLENDER_URL,
        json=payload,
...
def send_all_weights(*_):
    ...
    try:
        send_payload_to_blender(payload)
    except Exception as e:
        status_var.set(f"Blender error: {e}")


def set_shape_channel_value(channel_name, value, send_now=True):
    global is_programmatic_update

    if channel_name not in shape_slider_vars:
        return False

    value = max(0.0, min(1.0, float(value)))

    is_programmatic_update = True
    shape_slider_vars[channel_name].set(value)
    shape_value_labels[channel_name].config(text=f"{value:.2f}")
    if channel_name in shape_meta:
        shape_meta[channel_name]["last_value"] = value
    is_programmatic_update = False
...
def set_rotation_channel_value(channel_name, value, send_now=True):
    global is_programmatic_update

    if channel_name not in rotation_slider_vars:
        return False

    meta = rotation_meta[channel_name]
    min_value = meta["min"]
    max_value = meta["max"]
    value = max(min_value, min(max_value, float(value)))

    is_programmatic_update = True
    rotation_slider_vars[channel_name].set(value)
    rotation_value_labels[channel_name].config(text=f"{value:.2f}°")
    rotation_meta[channel_name]["last_value"] = value
    is_programmatic_update = False
...
def set_multiple_shape_channels(mapping, send_now=True):
    global is_programmatic_update
    changed = False

    is_programmatic_update = True
    for channel_name, value in mapping.items():
        if channel_name in shape_slider_vars:
            value = max(0.0, min(1.0, float(value)))
            shape_slider_vars[channel_name].set(value)
            shape_value_labels[channel_name].config(text=f"{value:.2f}")
            if channel_name in shape_meta:
...
def clear_vowel_channels(send_now=True):
    mapping = {}
    for v in ("a", "e", "i", "o", "u"):
        if v in shape_slider_vars:
            mapping[v] = 0.0
    if mapping:
        set_multiple_shape_channels(mapping, send_now=send_now)
...
def set_vowel_weight(vowel, weight, send_now=True):
    ...
    if vowel in shape_slider_vars:
        mapping[vowel] = weight
    elif "a" in shape_slider_vars:
        mapping["a"] = weight
    else:
        return False

    return set_multiple_shape_channels(mapping, send_now=send_now)
...
def reset_all():
    ...
    for name, var in shape_slider_vars.items():
        var.set(0.0)
        shape_value_labels[name].config(text="0.00")
        shape_meta[name]["base_value"] = 0.0
        shape_meta[name]["last_value"] = 0.0

    for name, var in rotation_slider_vars.items():
        var.set(0.0)
        rotation_value_labels[name].config(text="0.00°")
        rotation_meta[name]["base_value"] = 0.0
        rotation_meta[name]["last_value"] = 0.0
...
# BLINK
# ============================================================
def get_blink_channel():
    for alias in BLINK_ALIASES:
        if alias in shape_slider_vars:
            return alias
    return None

def do_blink():
    blink_channel = get_blink_channel()
    if not blink_channel or not blink_running:
        return
    set_shape_channel_value(blink_channel, 1.0, send_now=True)
...
def end_blink():
    ...
    if not blink_channel or not blink_running:
        return
    set_shape_channel_value(blink_channel, 0.0, send_now=True)
...
tb.Label(api_row, text="Blender URL", bootstyle="light").pack(side=LEFT, padx=(0, 8))
blender_url_var = tb.StringVar(value=BLENDER_URL)
tb.Entry(api_row, textvariable=blender_url_var, width=35).pack(side=LEFT, padx=(0, 16))
...
# SHAPE SLIDERS UI
# ============================================================
shapes_frame = tb.Labelframe(scrollable, text="Shape Keys", padding=10, bootstyle="info")
shapes_frame.pack(fill=X, padx=4, pady=5)

for name in SHAPE_CHANNELS:
    card = tb.Frame(shapes_frame, padding=10, bootstyle="secondary")
    ...
    shape_slider_vars[name] = slider_var
    shape_value_labels[name] = value_label
    ...
    shape_meta[name] = meta

    row = tb.Frame(card)
    row.pack(fill=X, pady=(8, 0))

    def make_shape_callback(channel_name):
        def _callback(value):
            v = float(value)
            shape_value_labels[channel_name].config(text=f"{v:.2f}")
            shape_meta[channel_name]["last_value"] = v
            if not shape_meta[channel_name]["anim_enabled_var"].get():
                shape_meta[channel_name]["base_value"] = v
    ...
    scale = tb.Scale(
        ...
        to=1.0,
        variable=slider_var,
        command=make_shape_callback(name),
        bootstyle="info"
    )
    scale.pack(fill=X)
    ...
    def make_shape_anim_toggle(channel_name):
        def _toggle():
            shape_meta[channel_name]["base_value"] = float(shape_slider_vars[channel_name].get())
        return _toggle
    ...
    tb.Checkbutton(
        ...
        command=make_shape_anim_toggle(name),
        bootstyle="round-toggle"
    ).pack(side=LEFT, padx=(0, 14))

    tb.Label(anim_row, text="Cycle seconds", bootstyle="light").pack(side=LEFT, padx=(0, 8))
...
# ROTATION SLIDERS UI
# ============================================================
rot_frame = tb.Labelframe(scrollable, text="Avatar Rotation", padding=10, bootstyle="info")
rot_frame.pack(fill=X, padx=4, pady=5)

for name, min_value, max_value in ROTATION_CHANNELS:
    card = tb.Frame(rot_frame, padding=10, bootstyle="secondary")
    card.pack(fill=X, padx=4, pady=5)
    ...
    slider_var = tb.DoubleVar(value=0.0)
    rotation_slider_vars[name] = slider_var
    rotation_value_labels[name] = value_label
    ...
    rotation_meta[name] = meta

    row = tb.Frame(card)
    row.pack(fill=X, pady=(8, 0))

    def make_rotation_callback(channel_name):
        def _callback(value):
            v = float(value)
            rotation_value_labels[channel_name].config(text=f"{v:.2f}°")
            rotation_meta[channel_name]["last_value"] = v
            if not rotation_meta[channel_name]["anim_enabled_var"].get():
                rotation_meta[channel_name]["base_value"] = v
            if not is_programmatic_update:
                send_all_weights()
        return _callback
    ...
    scale = tb.Scale(
        ...
        to=max_value,
        variable=slider_var,
        command=make_rotation_callback(name),
        bootstyle="info"
    )
    scale.pack(fill=X)
    ...
    def make_rotation_anim_toggle(channel_name):
        def _toggle():
            rotation_meta[channel_name]["base_value"] = float(rotation_slider_vars[channel_name].get())
        return _toggle
    ...
    tb.Checkbutton(
        ...
        text="Animate",
        variable=meta["anim_enabled_var"],
        command=make_rotation_anim_toggle(name),
        bootstyle="round-toggle"
    ).pack(side=LEFT, padx=(0, 14))
...
def ask_remote_ai(question):
    ...
    if not answer:
        raise RuntimeError(f"Unrecognized remote response: {data}")
...
def process_user_text(user_text):
    ...
    def worker():
        try:
            answer = ask_remote_ai(user_text)
            if not answer:
...
def animation_tick():
    try:
        ...
        # Shape animation: 0..base
        for channel_name, meta in shape_meta.items():
            ...
            current = float(shape_slider_vars[channel_name].get())
            if abs(current - value) > 0.002:
                set_shape_channel_value(channel_name, value, send_now=False)
                any_changed = True

        # Rotation animation: -base..+base
        for channel_name, meta in rotation_meta.items():
            if not meta["anim_enabled_var"].get():
                continue
            ...
            phase = (now % period) / period
            sine11 = math.sin(2.0 * math.pi * phase)
            value = base * sine11

            current = float(rotation_slider_vars[channel_name].get())
            if abs(current - value) > 0.01:
                set_rotation_channel_value(channel_name, value, send_now=False)
                any_changed = True
...
def send_test_question():
    process_user_text("Hola, ¿puedes presentarte brevemente?")

tb.Button(
    buttons_frame,
    text="Test AI",
    command=send_test_question,
...
def test_blender():
    try:
        global BLENDER_URL
        BLENDER_URL = blender_url_var.get().strip()
        r = requests.get(BLENDER_URL.replace("/update", "/status"), timeout=2)
        r.raise_for_status()
        status_var.set("Blender connection OK")
    except Exception as e:
        status_var.set(f"Blender status error: {e}")

tb.Button(
    buttons_frame,
    text="Test Blender",
    command=test_blender,
...
if get_blink_channel() is not None:
